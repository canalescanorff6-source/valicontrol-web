...
PIX_VALOR = float(os.getenv('PIX_VALOR', '100.00'))
PAGAMENTO_DESCRICAO = os.getenv('PAGAMENTO_DESCRICAO', 'Plano ValiControl PRO')
PAGAMENTO_MODO = os.getenv('PAGAMENTO_MODO', 'manual_pix').strip().lower()
PIX_CHAVE = os.getenv('PIX_CHAVE', '').strip()
PIX_TITULAR = os.getenv('PIX_TITULAR', '').strip()
PIX_WHATSAPP = os.getenv('PIX_WHATSAPP', os.getenv('CADASTRO_AUTORIZACAO_WHATSAPP', '')).strip()
PIX_OBSERVACAO = os.getenv('PIX_OBSERVACAO', 'Após pagar, envie o comprovante pelo WhatsApp para ativação manual do PRO.').strip()
